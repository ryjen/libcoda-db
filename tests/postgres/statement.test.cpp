...
SPEC_BEGIN(postgres_statement) {
  describe("postgres statement", []() {
    ...
    it("is movable", []() {
      postgres::statement stmt(dynamic_pointer_cast<postgres::session>(
      ...
      postgres::statement s3(dynamic_pointer_cast<postgres::session>(
          test::current_session->impl()));
    ...
    it("throws exceptions", []() {
      ...
      postgres::statement stmt(
          dynamic_pointer_cast<postgres::session>(session->impl()));
