...
void run_tests(const std::shared_ptr<coda::db::session> &session,
               const char *name);
...
int main(int argc, char *argv[]) {
  ...
  auto mysql_test_session =
      sqldb::create_session(get_env_uri("MYSQL_URI", "mysql://localhost/test"));
  auto postgres_test_session = sqldb::create_session(
      get_env_uri("POSTGRES_URI", "postgres://localhost/test"));

  auto sqlite_impl =
      dynamic_pointer_cast<test_session>(sqlite_test_session->impl());
  auto mysql_impl =
      dynamic_pointer_cast<test_session>(mysql_test_session->impl());
  auto postgres_impl =
      dynamic_pointer_cast<test_session>(postgres_test_session->impl());
...
void run_tests(const std::shared_ptr<coda::db::session> &session,
  ...
  log::info(
      "\033[1;35m%s\033[0m inserts took: \033[1;37m%ld millis\033[0m", name,
  ...
  log::info(
      "\033[1;35m%s\033[0m select all took: \033[1;37m%ld millis\033[0m", name,
      std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count());
}
