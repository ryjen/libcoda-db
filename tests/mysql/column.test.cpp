...
template <typename T>
void test_move_column(
    std::function<shared_ptr<column_impl>(size_t, size_t)> funk) {
  auto f1 = funk(0, 0);
  ...
  describe("mysql column", []() {
    ...
    describe("is movable", []() {
      it("as statement results",
         []() { test_move_column<mysql::stmt_column>(get_stmt_column); });

      it("as results",
         []() { test_move_column<mysql::column>(get_results_column); });
    });
    ...
    describe("can return a blob", []() {
      it("as statement results", []() {
        ...
        Assert::That(static_pointer_cast<mysql::stmt_column>(c)->sql_type(),
                     Equals(MYSQL_TYPE_BLOB));
