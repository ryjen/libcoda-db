...
void benchmark_insert(soci::session &session) {
  User user = {random_name(), random_name(),
               random_num<double>(-123012, 123123)};

  session << "insert into users(first_name, last_name, dval) "
             "values(:first_name, :last_name, :dval)",
      use(user.first_name), use(user.last_name), use(user.dval);
}

void create_table(soci::session &session) {
  session << "create table if not exists users(id integer primary key "
             "autoincrement, first_name varchar(45), last_name varchar(45), "
             "dval real, data "
             "blob, tval timestamp)";
}
...
void create_mysql_table(soci::session &session) {
  session << "create table if not exists users(id integer primary key "
             "auto_increment, first_name varchar(45), last_name varchar(45), "
             "dval real, data "
             "blob, tval timestamp)";
}

void create_postgres_table(soci::session &session) {
  session << "create table if not exists users(id serial primary key unique, "
             "first_name varchar(45), last_name varchar(45), dval real, "
