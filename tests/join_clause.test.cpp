...
specification(joins, []() {
  describe("a join", []() {
    before_each([&]() {
      ...
      insert_query query(test::current_session, "user_settings",
                         {"user_id", "valid", "created_at"});
    ...
    it("can be a cross join", []() {
      ...
      query.from(test::user::TABLE_NAME, "u")
          .join("user_settings", "s", join::cross)
          .on("u.id = s.user_id") and
          ("s.valid = 1");
    ...
    it("can be full outer", []() {
      if (test::current_session->has_feature(
          session::FEATURE_FULL_OUTER_JOIN)) {
        select_query query(test::current_session);
    ...
    it("can be a left", []() {
      ...
      query.from(test::user::TABLE_NAME, "u")
          .join("user_settings", "s", join::left)
          .on("u.id = s.user_id") and
          ("s.valid = 1");
    ...
    it("can be a inner", []() {
      ...
      query.from(test::user::TABLE_NAME, "u")
          .join("user_settings", "s", join::inner)
          .on("u.id = s.user_id") and
          ("s.valid = 1");
    ...
    it("can be a natural", []() {
      ...
      query.from(test::user::TABLE_NAME, "u")
          .join("user_settings", "s", join::natural);
    ...
    it("can be a default", []() {
      ...
      query.from(test::user::TABLE_NAME, "u")
          .join("user_settings", "s", join::none)
          .on("u.id = s.user_id") and
          ("s.valid = 1");
    ...
    it("can use a where clause", []() {
      ...
      Assert::That(join.on().to_sql(),
                   Equals("u.id = s.user_id AND s.valid = 1"));
    });
    ...
    it("can reset", []() {
      ...
      Assert::That(join.to_sql(),
                   Equals(" JOIN user_settings s ON u.id = s.user_id"));
