...
specification(selects, []() {
  describe("select query", []() {
    ...
    it("can be constructed with a schema", []() {
      ...
      select_query query(
          factory.get(test::current_session, test::user::TABLE_NAME));
    ...
    it("can execute scalar", []() {
      ...
      auto value = query.execute_scalar<string>();
    ...
    it("can union another", []() {
      ...
      Assert::That(
          query.to_sql(),
          Equals("SELECT * FROM users UNION SELECT * FROM user_settings;"));

      query.union_with(other, union_op::all);

      Assert::That(
          query.to_sql(),
          Equals("SELECT * FROM users UNION ALL SELECT * FROM user_settings;"));
    });
    ...
    it("can use different parameter types", []() {
      ...
      query.where("(first_name = ? and last_name = ?) or (first_name = ? or "
                  "last_name = ?) or last_name = @lname");
      query.bind(1, "Bob");
      query.bind(2, "Smith");
      query.bind(3, "Bryan");
      query.bind(4, "Smith");
      ...
      query.where("(first_name = $1 and last_name = $2) or (first_name = $3 or "
                  "last_name = $2) or last_name = @lname");
