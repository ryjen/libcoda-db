...
namespace arg3
{
    namespace db
    {
        ...
        void sqlite3_statement::prepare(const string &sql)
        {
            ...
            if (sqlite3_prepare_v2(db_->db_, sql.c_str(), -1, &temp, NULL) != SQLITE_OK) throw database_exception(db_->last_error());
        ...
        {
            if (sqlite3_bind_int(stmt_.get(), index, value) != SQLITE_OK) throw binding_error(db_->last_error());
            return *this;
        }
        sqlite3_statement &sqlite3_statement::bind(size_t index, long long value)
        {
            if (sqlite3_bind_int64(stmt_.get(), index, value) != SQLITE_OK) throw binding_error(db_->last_error());
            return *this;
        }
        sqlite3_statement &sqlite3_statement::bind(size_t index, double value)
        {
            if (sqlite3_bind_double(stmt_.get(), index, value) != SQLITE_OK) throw binding_error(db_->last_error());
            return *this;
        }
