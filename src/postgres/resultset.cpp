...
namespace coda {
  namespace db {
    namespace postgres {
      resultset::resultset(const std::shared_ptr<postgres::session> &sess,
                           const shared_ptr<PGresult> &stmt)
          : stmt_(stmt), sess_(sess), currentRow_(-1) {
        if (sess_ == nullptr) {
          throw database_exception(
              "No database provided to postgres resultset");
        }

        if (stmt_ == nullptr) {
          throw database_exception(
              "no statement provided to postgres resultset");
        }
      }
