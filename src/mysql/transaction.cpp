...
namespace coda {
  namespace db {
    namespace mysql {
      transaction::transaction(const std::shared_ptr<MYSQL> &db,
                               const transaction::mode &mode)
          : db_(db), mode_(mode), active_(false) {}

      transaction::transaction(const transaction &other)
          : db_(other.db_), mode_(other.mode_), active_(other.active_) {}

      transaction::transaction(transaction &&other)
          : db_(std::move(other.db_)), mode_(std::move(other.mode_)),
