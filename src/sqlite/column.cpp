    ...
    namespace sqlite {
      namespace data_mapper {
        sql_value to_value(const shared_ptr<sqlite3_stmt> &stmt, int column) {
          if (stmt == nullptr || column < 0 ||
              column >= sqlite3_column_count(stmt.get())) {
            return sql_value();
          }
          ...
          default: {
            const unsigned char *textValue =
                sqlite3_column_text(stmt.get(), column);
            if (textValue != NULL) {
      ...
      column::column(const shared_ptr<sqlite3_stmt> &stmt, int column)
          : stmt_(stmt), column_(column) {}

      column::column(column &&other)
          : stmt_(other.stmt_), column_(other.column_) {
        other.stmt_ = nullptr;
      }
