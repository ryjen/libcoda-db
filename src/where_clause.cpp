...
namespace coda::db {
  ...
  sql_operator::~sql_operator() {
    switch (type_) {
      ...
      case op::LESSER:
      case op::EQ_GREATER:
      case op::EQ_LESSER:rvalue_.~sql_value();
        break;
      case op::IN:rvalues_.~vector<sql_value>();
        break;
      case op::BETWEEN:rrange_.~pair<sql_value, sql_value>();
        break;
      default:break;
    }
  }
  ...
  void sql_operator::copy(const sql_operator &other) {
    ...
    switch (type_) {
      ...
      case op::LESSER:
      case op::EQ_GREATER:
      case op::EQ_LESSER:new(&rvalue_) sql_value(other.rvalue_);
        break;
      case op::IN:new(&rvalues_) std::vector<sql_value>(other.rvalues_);
        break;
      case op::BETWEEN:new(&rrange_) std::pair<sql_value, sql_value>(other.rrange_);
        break;
      default:break;
    }
  }
  ...
  void sql_operator::move(sql_operator &&other) noexcept {
    ...
    switch (type_) {
      ...
      case op::LESSER:
      case op::EQ_GREATER:
      case op::EQ_LESSER:new(&rvalue_) sql_value(std::move(other.rvalue_));
        break;
      case op::IN:new(&rvalues_) std::vector<sql_value>(std::move(other.rvalues_));
        break;
      case op::BETWEEN:new(&rrange_) std::pair<sql_value, sql_value>(std::move(other.rrange_));
        break;
      default:break;
    }
  }
  ...
  where_builder &where_builder::bind(size_t index, const sql_operator &value) {
    switch (value.type_) {
      ...
      case op::IN:
        ...
        break;
      case op::BETWEEN:binder_->bind(index, value.rrange_.first);
