...
namespace coda {
  namespace db {
    ...
    namespace impl {
      using factory_storage =
          std::unordered_map<std::string, std::shared_ptr<session_factory>>;
    ...
    std::shared_ptr<session> create_session(const uri &uri) {
      ...
      if (factory == nullptr) {
        throw database_exception("unregistered database protocol: " +
                                 uri.protocol);
      }
    ...
    void register_session(const std::string &protocol,
      ...
      if (protocol.empty()) {
        throw database_exception(
            "invalid protocol for session factory registration");
      }

      if (factory == nullptr) {
        throw database_exception(
            "invalid factory for session factory registration");
      }
