...
namespace coda {
  namespace db {
    ...
    class sql_value : public sql_value_convertible {
      ...
      private:
      std::variant<sql_null_type, sql_number, sql_string, sql_wstring, sql_time,
                   sql_blob>
          value_;
    };

    template <> sql_string sql_value::as<sql_string>() const;

    template <> sql_wstring sql_value::as<sql_wstring>() const;

    template <> sql_time sql_value::as<sql_time>() const;

    template <> sql_blob sql_value::as<sql_blob>() const;

    template <> sql_number sql_value::as<sql_number>() const;
