...
namespace arg3
{
    namespace db
    {
        ...
        sqlite3_db::sqlite3_db(const sqlite3_db &other)
            : sqldb(other), db_(other.db_), filename_(other.filename_), schema_factory_(other.schema_factory_)
        {
        }

        sqlite3_db::sqlite3_db(sqlite3_db &&other)
            : sqldb(other), db_(std::move(other.db_)), filename_(std::move(other.filename_)), schema_factory_(std::move(other.schema_factory_))
        {
        }
        ...
        void sqlite3_db::open(int flags)
        {
            ...
            if (sqlite3_open_v2(filename_.c_str(), &db_, flags, NULL) != SQLITE_OK) throw database_exception(last_error());
        }
